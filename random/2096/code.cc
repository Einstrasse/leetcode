...
class Solution {
public:
    bool findNode(TreeNode* current, int target, vector<pair<char, int>>& path) {
        bool ret = false;
        ...
        if (current->left != nullptr) {
            path.push_back(make_pair('L', current->left->val));
        ...
        if (current->right != nullptr) {
            path.push_back(make_pair('R', current->right->val));
    ...
    string getDirections(TreeNode* root, int startValue, int destValue) {
        vector<pair<char, int>> startPath, destPath;
        
        findNode(root, startValue, startPath);
        findNode(root, destValue, destPath);
        int index = 0;
        //reduce common ancestors
        while (index < startPath.size() && index < destPath.size() && startPath[index].second == destPath[index].second) index++;
        string ans = "";
        for (int i = index; i < startPath.size(); i++) {
            ans += "U";
        }
        for (int i= index; i < destPath.size(); i++) {
            ans += destPath[i].first;
        }
        return ans;
    }
};
